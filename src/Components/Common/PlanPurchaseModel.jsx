...
import { CREATE_SUBSCRIPTION, GET_ALL_CARD, PAYMENT_DETAILS, UPGRADE_SUBSCRIPTION, paypalOptions } from '../../Pages/Api';
...
const PlanPurchaseModel = ({ selectPlan, discountCoupon, clientSecret, Screen, setOpenPayment, openPayment, userPlanType, purchaseType, setPurchaseType }) => {
    ...
    const handlePay = () => {
        if (selectCard !== "") {
            cardMethod = selectCard?.paymentMethod
            ...
            // }
                ...
                UpgradeSubscription({ email: user?.emailID, name: user?.userDetails?.firstName + " " + user?.userDetails?.lastName, PaymentMethodId: selectCard?.paymentMethodID, paymentIntent: selectCard?.paymentMethod, organizationID: user?.organizationId })
            }
        }
    }
