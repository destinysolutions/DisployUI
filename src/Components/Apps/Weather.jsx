...
const Weather = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  const handleUpdateTagsWeather = (tags) => {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${WEATHER_ADD_TAG}WeatherId=${updateTagWeather?.weatherAppId
  ...
  return (
    <>
      ...
      <div className={user?.isTrial && user?.userDetails?.isRetailer === false && !user?.isActivePlan ?"lg:pt-32 md:pt-32 pt-10 px-5" : "lg:pt-24 md:pt-24 pt-10 px-5 "}>
                ...
                {loadingModel ? (
                  ...
                  >
                    <div className="lg:mx-auto md:mx-auto lg:max-w-5xl md:max-w-3xl sm:max-w-xl xs:w-full mx-auto bg-teal border-width-10px border-black">
                      <div
                        className={`flex ${Layout === "Landscape" ? "flex-row" : "flex-col"
                          } text-[#ffffff]`}
                      >
