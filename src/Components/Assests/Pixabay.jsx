...
const Pixabay = ({ closeModal }) => {
    ...
    useEffect(() => {
        ...
        let url = `${BASE_URL}&q=${encodeURIComponent(searchTerm)}&page=${currentPage}`;
    ...
    const handleImageUpload = () => {
        ...
        selectedImages.forEach((image) => {
            ...
            formData.append("File", image.webformatURL);
            ...
            <div className="backdrop">
                <div className="fixed unsplash-model bg-black lg:px-5 md:px-5 sm:px-3 xs:px-2 py-7 rounded-2xl">
                    ...
                    <div className="grid grid-cols-12 px-3 gap-4 unsplash-section bg-white rounded-lg">
                        {images.map((image) => (
                            <div
                                ...
                                style={{
                                    border: selectedImages.includes(image) ? "2px solid blue" : "2px solid white",
                                }}
                            >
                                <img src={image.webformatURL} alt={image.tags} className=" h-full w-full object-cover" />
                            </div>
                        ))}
                    </div>
                    ...
                    {selectedImages.length > 0 && (
                        <div className="mt-4 text-center">
                            <button onClick={handleImageUpload} className="px-4 py-2 bg-blue-500 text-white rounded-md">
