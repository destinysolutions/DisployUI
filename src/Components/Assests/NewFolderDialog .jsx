...
const NewFolderDialog =({ sidebarOpen, setSidebarOpen }) => {
  ...
  const loadEventsForSchedule = (folderId) => {
    axios
      .get(`${FetchdataFormFolder}?ID=${folderId}`)
      .then((response) => {
    ...
    <div className={`${sidebarOpen ? "ml-60" : "ml-0"}`}>
      ...
      <div className=" mt-4 page-content grid lg:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 xs:grid-cols-1 gap-8 mb-10 assets-section">
        {data.images.map((image) => (
          <><div key={image.id} className='relative list-none assetsbox'>
            <img src={image.fileType} alt={image.name} className='relative imagebox w-full rounded-2xl' />

            <div className="tabicon text-center absolute left-[50%] bottom-[0px]" onMouseEnter={() => setHoveredTabIcon(image)}
         ...
         {assetsdw === image && selectedItems.includes(image) && (
        ...
        {data.videos.map((video) => (
          <div key={video.id} className='relative list-none assetsbox'>
            ...
            <div className="tabicon text-center absolute left-[50%] bottom-[0px]" onMouseEnter={() => setHoveredTabIcon(video)}
            onMouseLeave={() => setHoveredTabIcon(null)}
            ...
            {assetsdw === video && selectedItems.includes(video) && (
              <div className="assetsdw">
                <ul>
                  <li className="flex text-sm items-center">
                    <FiUpload className="mr-2 text-lg" />
        ...
        {data.documents.map((document) => (
          <><div key={document.id} className="bg-white px-4 py-5 rounded-lg shadow-lg h-full relative">
            ...
            <div className="checkbox flex justify-between absolute top-5 px-4 w-full">
              <input
                type="checkbox"
                className="w-[20px] h-[20px]"
                checked={selectAll || selectedItems.includes(document)}
                onChange={() => handleCheckboxChange(document)} />
              <button onClick={() => updateassetsdw(document)}>
                <BsThreeDots className="text-2xl" />
              </button>
              {assetsdw === document && selectedItems.includes(document) && (
                <div className="assetsdw">
                  <ul>
                    <li className="flex text-sm items-center">
                      <FiUpload className="mr-2 text-lg" />
